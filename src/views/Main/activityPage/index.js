...
const ActivityPage = props => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (city === "all") {
        const response = await PTX.get(`/v2/Tourism/Activity?${term === "all" ? "" : `$filter=contains(ActivityName,'${term}')`}`, {
          params: {
            $format: "JSON",
            $top: 100,
          },
      ...
      if (city !== "all") {
        const response = await PTX.get(`/v2/Tourism/Activity?${term === "all" ? `$filter=contains(city,'${city}') or contains(Address,'${city}')` : `$filter=contains(ActivityName,'${term}') and (contains(city,'${city}') or contains(Address,'${city}'))`}`, {
          params: {
            $format: "JSON",
          },
        });
  ...
  return (
    <main>
      <div className="main__spotPage">
        ...
        <h3>找活動</h3>
        {/* //! */}
        <SearchBar type="activity" />
        {<p>搜尋結果：{`關鍵字：${term}，城市：${city}，${amount}個結果`}</p>}
      </div>
      <SearchList type="activity" className="main__searchList" data={dataObj} hash={props.match.params} />
      <PageBtnBar type="activity" className="main__pageBtnBar" hash={props.match.params} />
    </main>
  );
};
